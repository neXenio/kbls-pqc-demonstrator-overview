...
To better illustrate the concept and verify its implementation, scenarios with test vectors are described here.

In the following, Alice has the user password `password123456` as well as a Kyber and an RSA key pair:
...
## Alice creates key material for a board

Alice creates key material for a new board, which she initially encrypts only for herself.

1. Alice generates a board key (32 random bytes):
<details>
...
2. Alice creates and encrypts an initial secret with Kyber, which later serves as KDF input:
<details>
  <summary>Source Code</summary>
...
3. Alice creates and encrypts a second secret using RSA, which later serves as KDF input:
<details>
  <summary>Source Code</summary>
...
4. Alice encrypts the board key
<details>
  <summary>Source Code</summary>
...
## Alice opens a board she previously created

The steps in which Alice obtains data from the server are omitted here for simplicity.

1. Alice already has the encrypted key material
...
2. Alice decrypts the first secret with Kyber, which later serves as KDF input:
...
3. Alice decrypts the second secret with RSA, which later serves as KDF input:
...
4. Alice decrypts the board key:
